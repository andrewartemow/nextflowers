        ...
        <Sheet open={shouldDisplayCart} onOpenChange={handleCartClick}>
            <SheetContent className="sm:max-w-lg w-[90vw]">
                ...
                <div className="h-full flex-col justify-between">
                    <div className="mt-8 flex-1 h-[65%] overflow-y-auto">
                        <ul className="-my-6 divide-y divide-gray-200">
                            {cartCount === 0 ? (
                                <h2 className="py-6">You dont have any items</h2>
                            ) : <>
                                {Object.values(cartDetails ?? {}).map(value =>
                                    <li key={value.id} className="flex py-6">
                                        <div className="h-24 w-24 flex-shrink-0 overflow-hidden rounded-md border border-gray-200">
                                            <Image src={value.image as string} alt="Product Image" width={100} height={100} />
                                        </div>
                                        <div className="ml-4 flex flex-1 flex-col">
                                            <div>
                                                ...
                                                <p className="mt-1 text-sm text-gray-500 line-clamp-2">{value.description}</p>
                                            </div>
                                            <div className="flex flex-1 items-end justify-between text-sm">
                                                <p className="text-gray-500">QTY: {value.quantity}</p>

                                                <div className="flex">
                                                    <button type="button" className="font-medium text-primary hover:text-primary/80" onClick={() => removeItem(value.id)}>
                                                        Remove
                                                    </button>
                                                </div>
                                            </div>
